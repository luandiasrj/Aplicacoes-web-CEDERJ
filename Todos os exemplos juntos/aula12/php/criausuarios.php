...
function insere_usuario ($nome, $senha, $conexao) {
   ...
   // mysql_query ("insert into usuarios values (\"$nome\", \"$senha_enc\")", $conexao) or
   mysqli_query ($conexao, "insert into usuarios values (\"$nome\", \"$senha_enc\")") or
...
$consulta = mysqli_query ($conexao, "DROP TABLE IF EXISTS usuarios") or die ("Erro dropping");
// $consulta = mysql_query ("CREATE TABLE usuarios (nome varchar(10) not null, senha varchar(15) not null, PRIMARY KEY (nome))") or die ("Erro criando");
$consulta = mysqli_query ($conexao, "CREATE TABLE usuarios (nome varchar(10) not null, senha varchar(15) not null, PRIMARY KEY (nome))") or die ("Erro criando");
